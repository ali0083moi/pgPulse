      ...
      <div className="w-full max-w-3xl bg-[#161B22] border border-[#30363D] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-[#30363D] flex items-center justify-between bg-[#161B22] shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-[#F0F6FC]">Visually Create New PostgreSQL Table</h2>
              <p className="text-xs text-[#8B949E]">Define table structure, data types, primary keys & foreign keys</p>
            </div>
          </div>
        ...
        <form onSubmit={handleCreateTable} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-[#8B949E]">Table Name <span className="text-[#F85149]">*</span></label>
            <input
              type="text"
              required
              value={tableName}
              ...
              className="w-full px-3 py-2 rounded-xl bg-[#0D1117] border border-[#30363D] text-sm text-[#C9D1D9] placeholder-[#484F58] focus:outline-none focus:border-[#58A6FF] font-mono font-bold"
            />
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-[#8B949E] uppercase tracking-wider">Columns</span>
              <button
                type="button"
                onClick={handleAddColumn}
                className="px-3 py-1 rounded-lg bg-[#21262D] hover:bg-[#30363D] border border-[#30363D] text-[#58A6FF] text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
              >
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {columns.map((c) => (
                <div key={c.id} className="flex items-center gap-2 p-2 rounded-xl bg-[#0D1117] border border-[#30363D] text-xs">
                  <input
                    type="text"
                    required
                    value={c.name}
                    ...
                    className="w-36 px-2.5 py-1 rounded-lg bg-[#161B22] border border-[#30363D] text-[#C9D1D9] font-mono text-xs focus:outline-none focus:border-[#58A6FF]"
                  />
                  ...
                  <label className="flex items-center gap-1 text-[11px] text-[#8B949E] cursor-pointer select-none px-1">
                    <input
                      type="checkbox"
                      checked={!!c.isPrimaryKey}
                      onChange={(e) => handleUpdateColumn(c.id, 'isPrimaryKey', e.target.checked)}
                      className="rounded border-[#30363D] bg-[#161B22] text-[#D29922] focus:ring-0"
                    />
                  ...
                  <label className="flex items-center gap-1 text-[11px] text-[#8B949E] cursor-pointer select-none px-1">
                    <input
                      ...
                      className="rounded border-[#30363D] bg-[#161B22] text-[#58A6FF] focus:ring-0"
                    />
                    NN
                  </label>
                  ...
                  <input
                    ...
                    className="flex-1 px-2 py-1 rounded-lg bg-[#161B22] border border-[#30363D] text-[#C9D1D9] font-mono text-[11px] focus:outline-none focus:border-[#58A6FF]"
                  />
          ...
          <div className="space-y-3 pt-2 border-t border-[#30363D]">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-[#8B949E] uppercase tracking-wider flex items-center gap-1.5">
                <LinkIcon className="w-3.5 h-3.5 text-[#58A6FF]" />
              ...
              <button
                ...
                className="px-3 py-1 rounded-lg bg-[#21262D] hover:bg-[#30363D] disabled:opacity-40 text-[#C9D1D9] text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
              >
                <Plus className="w-3.5 h-3.5" />
                Add FK Constraint
              </button>
            ...
            {tablesList.length === 0 && foreignKeys.length === 0 && (
              <p className="text-[11px] text-[#8B949E] italic">No existing tables in this schema to reference for Foreign Keys.</p>
            )}
            ...
            {foreignKeys.map((fk) => {
              ...
              return (
                <div key={fk.id} className="flex items-center gap-2 p-2 rounded-xl bg-[#0D1117] border border-[#30363D] text-xs">
                  <select
                    value={fk.columnName || ''}
                    onChange={(e) =>
                      setForeignKeys(foreignKeys.map((item) => (item.id === fk.id ? { ...item, columnName: e.target.value } : item)))
                    }
                    className="px-2 py-1 rounded-lg bg-[#161B22] border border-[#30363D] text-[#58A6FF] font-mono text-xs focus:outline-none"
                  >
                  ...
                  <select
                    ...
                    className="px-2 py-1 rounded-lg bg-[#161B22] border border-[#30363D] text-[#3FB950] font-mono text-xs focus:outline-none"
                  >
                  ...
                  <select
                    ...
                    className="px-2 py-1 rounded-lg bg-[#161B22] border border-[#30363D] text-[#C9D1D9] text-xs focus:outline-none ml-auto"
                  >
          ...
          <div className="space-y-1.5 pt-2 border-t border-[#30363D]">
            ...
            <pre className="p-3 rounded-xl bg-[#0D1117] border border-[#30363D] text-[#58A6FF] font-mono text-xs overflow-x-auto whitespace-pre-wrap max-h-36">
              {generatedSql}
            </pre>
          </div>
        </form>
